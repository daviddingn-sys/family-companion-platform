...
前端会把中国大陆 11 位手机号自动转换为 `+86` 格式后提交给 Supabase Auth，并使用短信验证码完成注册和登录。当前平台不再使用邮箱注册、邮箱验证、密码登录和 `/auth/callback` 回调。

面向国内用户正式上线时，建议优先使用 Supabase Auth 的 Send SMS Hook 接入阿里云短信或腾讯云短信，而不是直接使用 Twilio。这样短信签名、模板备案和国内到达率更可控。
