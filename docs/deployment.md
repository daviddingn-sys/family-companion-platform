...
前端只展示手机号和密码。为了避免依赖短信平台，服务端会把中国大陆手机号转换为内部系统邮箱，再用 Supabase Email+Password Auth 创建和登录账号。这个系统邮箱不暴露给用户，也不作为产品账号入口。

Supabase Auth 不需要开启 Phone provider，也不需要配置 Twilio。当前平台不使用邮箱注册入口、邮箱验证、短信验证码登录和 `/auth/callback` 回调。

当前阶段不依赖短信平台。已登录用户可在个人资料页主动修改密码；忘记密码仍先由管理员重置。后续正式上线前再接入阿里云短信或腾讯云短信，用于手机号验证、短信验证码登录和找回密码。
